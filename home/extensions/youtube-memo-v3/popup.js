...
function setupEventListeners() {
  ...
  document.getElementById('video-memo-list').addEventListener('click', async (e) => {
    const addFirstMemoBtn = e.target.closest('.add-first-memo');
    if (addFirstMemoBtn) {
      console.log('첫 메모 추가 버튼 클릭됨');
      const videoId = addFirstMemoBtn.dataset.videoId;
      await showDetailView(videoId, true);
    }
  });
...
async function loadMemoList(forceList = false) {
  ...
  try {
    ...
    // 데이터가 없고 YouTube 페이지가 아닌 경우
    if (Object.keys(allData).length === 0 && !isYouTube) {
      videoMemoList.innerHTML = `
        <div class="empty-state">
          <p>저장된 메모가 없습니다.</p>
          <button id="add-new-page" class="primary-button">새 페이지 추가하기</button>
        </div>`;
      
      document.getElementById('add-new-page').addEventListener('click', showAddPageForm);
      return;
    }

    // YouTube 페이지이지만 현재 페이지의 메모가 없는 경우
    if (isYouTube && currentVideoId && !allData[currentVideoId]) {
      console.log('새 비디오 페이지 감지됨:', currentVideoId);
      const videoInfo = await getVideoInfo(currentTab);
      const addNewElement = document.createElement('div');
      addNewElement.className = 'video-item new-video';
      addNewElement.innerHTML = `
        <div class="video-info">
          <div class="video-title">${videoInfo.title || '현재 동영상'}</div>
          <div class="video-channel">${videoInfo.channel || ''}</div>
        </div>
        <button class="add-first-memo" data-video-id="${currentVideoId}">
          첫 메모 추가하기
        </button>
      `;
      videoMemoList.appendChild(addNewElement);
      
      // 첫 메모 추가 버튼에 직접 이벤트 리스너 추가
      addNewElement.querySelector('.add-first-memo').addEventListener('click', async () => {
        console.log('첫 메모 추가 버튼 클릭됨 (직접)');
        await showDetailView(currentVideoId, true);
      });
    }
...
// 메모 상세 화면 표시 함수 수정
async function showDetailView(videoId, isNewMemo = false) {
  console.log('showDetailView 호출됨:', videoId, isNewMemo);
  ...
  try {
    ...
    // 비디오 제목 설정
    document.getElementById('video-title').textContent = videoInfo.title || '제목 없음';
    ...
    if (isNewMemo) {
      const memoInput = document.getElementById('memo-input');
      memoInput.focus();
      // 기존 메모가 없는 경우 빈 목록 표시
      document.getElementById('memo-list').innerHTML = 
        '<div class="empty-state">첫 메모를 추가해보세요!</div>';
    } else {
      // 기존 메모 로드
  ...
  videoMemoList.innerHTML = `
    <div class="add-page-form">
      ...
      <div class="form-group">
        <label for="video-url">YouTube 동영상 URL</label>
        <input type="text" id="video-url" placeholder="https://www.youtube.com/watch?v=...">
      </div>
      <div class="form-actions">
        <button id="cancel-add-page" class="secondary-button">취소</button>
        <button id="submit-add-page" class="primary-button">추가</button>
      </div>
    </div>
  `;

  // 취소 버튼
  document.getElementById('cancel-add-page').addEventListener('click', () => {
    loadMemoList(true);
  });

  // 추가 버튼
  document.getElementById('submit-add-page').addEventListener('click', async () => {
    ...
    try {
      ...
      if (!videoId) {
        throw new Error('올바른 YouTube 동영상 URL을 입력해주세요.');
      }
      ...
      chrome.tabs.create({ url: url }, (tab) => {
        showToast('새 페이지가 추가되었습니다. 메모를 작성할 수 있습니다.');
      });
